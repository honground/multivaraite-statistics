...
for(name in names(data.new)){
  if((name == "car.name") || (name=="origin")||(name=="cylinders")||(name=='mpg')||(name=="model.year")){
  }
  else{
  png(filename = paste("scatterplot",name,".png"))
  plot(data.new[[name]],data.new$mpg, main=paste(name,"vs mpg"))
  dev.off()
  }
}
data.new2 <- subset(data.new, select = -c(horsepower))
lm.new2 <- lm(mpg~., data = data.new2)
...
for(name in names(data.new3)){
  if((name == "car.name") || (name=="origin")||(name=="cylinders")||(name=='mpg')||(name=="model.year")){
    ...
    png(filename = paste("scatterplot",name,".png"))
    plot(data.new3[[name]],data.new3$mpg, main=paste(name,"vs mpg"))
    dev.off()
  }
}
png(filename = "residual vs fitted.png")
